...
export async function getUpdateUsername(fastify: FastifyInstance, request: FastifyRequest, reply: FastifyReply) {
	try {
		...
		if (newUsername.length > 16 || newUsername.length < 1)
			return reply.send({ ok: false, message: "Username length invalid! 1-16 characters" });

		const valid = /^[a-zA-Z0-9_]+$/.test(newUsername);
		if (!valid)
			return reply.send({ ok: false, message: "Username can only contain letters, numbers and underscores!" });;

		const isMatch = await bcrypt.compare(password, user.password);
		if (!isMatch) {
			return reply.send({ ok: false, message: "Wrong password. Please try again!" });
		}
...
export async function getUpdateEmail(fastify: FastifyInstance, request: FastifyRequest, reply: FastifyReply) {
	try {
		...
		if (duplicate?.email === newE) {
			return reply.send({ ok: false, message: "Email already in use."});
		}

		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newE))
			return reply.send({ ok: false, message: "Invalid email format." });;
...
export async function getUpdatePassword(fastify: FastifyInstance, request: FastifyRequest, reply: FastifyReply) {
	try {
		...
		return reply.send({ field: (err as any).field ?? null, ok:false, message: (err as Error).message });
	}
}
...
export async function getUploadAvatar(request: FastifyRequest, reply: FastifyReply) {
	...
	if (!avatar?.filename) {
		return reply.send({ ok: false, error: "Nothing uploaded"});
	}
	if (!ALLOWED_MIME.includes(avatar.mimetype)) {
		return reply.send({ ok: false, error: "Bad file type, png/jpeg only" });
	}
	const image = mime.lookup(avatar.filename);
	if (!image)
		return reply.send({ ok: false, error: "Bad file"})
	const type =  mime.extension(image);
	if (!type) {
		return reply.send({ ok: false, error: "Cannot detect file extension" });
	}
	...
	if (avatar.file.truncated || avatar.file.bytesRead > MAX_SIZE) {
		...
		return reply.send({ ok: false, error: "File too large (max 2MB)" });
	}
...
export async function getUpdateStatus(request: FastifyRequest, reply: FastifyReply) {
	...
	if (!allowed.includes(status)) {
		return reply.send({ ok: false, error: "Invalid status" });
	} 
	if (request.user!.user_id !== null) {
...
export async function deleteUser(fastify: FastifyInstance, request: FastifyRequest, reply: FastifyReply) {
	try {
		...
		if (user.twofa_enabled)
			return reply.send({ message: "2FA must be disabled first" });
		if (confirmUser.toString() !== ("DELETE " + user.pseudo).toString())
			return reply.send({ message: "Please confirm the account deletion by writing \"DELETE username\" !" });
		const isMatch = await bcrypt.compare(password, user.password);
		if (!isMatch) {
			return reply.send({ message: "Wrong password. Please try again!" });
		}
